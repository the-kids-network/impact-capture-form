...
class RecreateReportingMentorsAggViewIgnoringDeletedSessionReports extends Migration
{
    /**
     * Run the migrations.
     *
...
SELECT DISTINCT
    ...
    COALESCE((SELECT true WHERE u.deleted_at IS NULL), false) AS active
FROM users u
LEFT JOIN (
...
LEFT JOIN (
    /* join planned session data */
    SELECT DISTINCT
        m.mentor_id,
        MIN(s.date) as next_planned_session_date
    FROM mentees m   
    JOIN planned_sessions s ON m.id=s.mentee_id
    WHERE s.date >= now()
...
WHERE 1=1
AND u.role IS NULL
AND u.permenantly_deleted = false;
SQL;
    }
