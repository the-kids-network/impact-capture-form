describe('Expense claim submission', () => {
    describe('For mentor', () => {
        ...
        it('Succeeds for full data', () => {
            ...
            cy.get('.expense-items tr').eq(1).find('.expense-date').should('contain', 'May 28, 2020')
            cy.get('.expense-items tr').eq(1).find('.expense-description').should('contain', 'Cinema Ticket')
            cy.get('.expense-items tr').eq(1).find('.expense-amount').should('contain', '1.5')
            cy.get('.expense-items tr').eq(2).find('.expense-date').should('contain', 'May 28, 2020')
            cy.get('.expense-items tr').eq(2).find('.expense-description').should('contain', 'Tube Ticket')
            cy.get('.expense-items tr').eq(2).find('.expense-amount').should('contain', '5.25')
