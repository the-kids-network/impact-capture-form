                                    ...
                                    <div class="container">
                                        <div class="tkn-list-group-item row mentor"> 
                                            ...
                                            <div class="col-12 order-3 col-md-6 order-md-3">
                                                <span>
                                                    @if($mentor->trashed())
                                                        <form style="display: inline-block" action="{{ url('/users/'.$mentor->id.'/restore') }}" id="restore-{{$mentor->id}}" method="post">
                                                            {{ csrf_field() }}
                                                            <a href="javascript:{}" class="btn btn-link" onclick="document.getElementById('restore-{{$mentor->id}}').submit(); return false;">Restore</a>
                                                        </form>
                                                        |
                                                        <form style="display: inline-block" action="{{ url('/users/'.$mentor->id) }}" id="delete-{{$mentor->id}}" method="post">
                                                            {{ csrf_field() }}
                                                            {{ method_field('DELETE') }}
                                                    ...
                                                    @else
                                                        <form style="display: inline-block" action="{{ url('/users/'.$mentor->id) }}" id="deactivate-{{$mentor->id}}" method="post">
                                                            {{ csrf_field() }}
                                                            {{ method_field('DELETE') }}
                                    ...
                                    <div class="container collapse mentor_mentees" id="mentees_for_mentor_{{$mentor->id}}">
                                        ...
                                        <div class="row tkn-list-group-item mentee">       
                                            ...
                                            <div class="col-12 col-md-6">
                                                <form action="/users/{{ $mentor->id }}/mentees/{{ $mentee->id }}" method="post" id="disassociate-mentor{{$mentor->id}}-mentee{{$mentee->id}}">
                                                    {{ csrf_field() }}
                                                    {{ method_field('delete') }}
...
@section('body-scripts')
    ...
    @include('user_management.include.mentor_mentee_pairing_scripts')
